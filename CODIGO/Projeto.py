...
# 
# Através de diversas técnicas podemos encontrar e remover informações desnecessárias de uma imagem, diminundo o espaço ocupado em disco por esta mas mantendo uma qualidade bem próxima da original, com diferenças na maioria das vezes impercepitiveis. A transformada discreta de Fourier e a segmentação  podem ser usadas como ferramentas para se atingir este objetivo.
# 
# 
# Há basicamente dois tipos de técnicas de compressão de imagens: Com Perda e Sem Perda. As técnicas sem perda são usadas quando se quer temporariamente reduzir a informação, Enquanto as técnica com perda são usadas quando se quer permanente reduzir o tamanho de uma imagem. <cite data-cite = "s8"> </cite>

# ## O Formato JPEG

# O termo JPEG é um acrônimo para Joint Photographic Experts Group, um comitê que tem uma longa tradição na criação de padrões de codificação de imagens sem movimento <cite data-cite = "s8"> </cite> .
# 
...
# $$\alpha[k] = \sqrt{\frac{2}{N}} \;caso\;contrário$$
...
# $$ C(\gamma) = \sqrt{\frac{2}{N}} \;caso\;contrário$$
# 
...
# Função que retorna a taxa de compressão (Compressio Rate)  de uma imagem

# In[146]:


def CR(pathOrig, pathCmp):
    result = stat(pathCmp).st_size/stat(pathOrig).st_size
    return result
...
# A taxa de compressão de uma imagem pode ser obitida através da seguinte fórmula:
# 
# $$C = \frac{n_c}{n_o}$$
# 
# onde:
...
# 
# $$C = \frac{697,977 bytes}{1,131,392 bytes} = 0.00532273$$

# A taxa de compressão da imagem foi de um pouco mais de 5 por cento, o que é um valor pequeno para apenas uma imagem, mas caso precissassemos armazenar milhares de imagens  essa diferença se acumularia fazendo o processo valer a pena. 
# 
...
# As medidas de tendência central apresentaram uma taxa de compressão bem próxima de 50 por cento, e PSNR em torno de 32, mostrando que as imagens grandes sofreram uma diminuição significante em seu tamanho e mantiveram uma qualidade considerável, porém as medidas de desvio padrão deram valores consideravelmente altos. A partir disto conclui-se que dependendo das propriedades da imagem pode-se obter uma taxa de compressão bem pequena e que imagens maiores tendem a obter um melhor CR (Compression Rate). 
